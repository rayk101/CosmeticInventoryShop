...
if (isset($_SESSION['login'])) {
    ...
    $CosmeticsTypeCode = trim(htmlspecialchars($_POST['CosmeticsTypeCode'] ?? ''));
    $CosmeticsTypeName = trim(htmlspecialchars($_POST['CosmeticsTypeName'] ?? ''));

    if ($CosmeticsTypeID === false || $CosmeticsTypeID === null) {
        echo "<h2>Sorry, you must enter a valid type ID number</h2>\n";
    }
    else if (empty($CosmeticsTypeCode) || empty($CosmeticsTypeName)) {
        echo "<h2>Sorry, you must enter both a type code and type name</h2>\n";
    }
